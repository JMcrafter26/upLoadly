...
<html>
	...
	<body>
		...
		<style>#footer{padding:10px 0 0 0;text-align:center;margin-top:20px;cursor:pointer;font-size:12px}#footer svg{transition:all .25s ease-in-out;font-size:12px;height:12px}#footer:hover svg{animation:electric .5s infinite;animation-timing-function:ease-in-out}#footer a{transition:color .25s ease-in-out}#footer:hover a{color:#fedd48;text-decoration:none}@keyframes electric{0%{filter:url(#sharp-drop-shadow) drop-shadow(0 0 7px rgb(254, 221, 72, .7));transform:rotate(0)}25%{filter:url(#sharp-drop-shadow) drop-shadow(0 0 10px #fedd48);transform:rotate(2.5deg)}50%{filter:url(#sharp-drop-shadow) drop-shadow(0 0 12px #fedd48);transform:rotate(5deg)}75%{filter:url(#sharp-drop-shadow) drop-shadow(0 0 15px #fedd48);transform:rotate(2.5deg)}100%{filter:url(#sharp-drop-shadow) drop-shadow(0 0 16px rgb(254, 221, 72, .7));transform:rotate(0)}}</style><div id=footer><svg class="feather feather-zap"fill=none stroke=currentColor stroke-linecap=round stroke-linejoin=round stroke-width=2 viewBox="0 0 24 24"xmlns=http://www.w3.org/2000/svg><defs><filter height=200% id=sharp-drop-shadow width=200% x=-50% y=-50%><feGaussianBlur in=SourceAlpha stdDeviation=1 /><feOffset dx=0 dy=0 result=offsetblur /><feFlood flood-color=#fedd48 /><feComposite in2=offsetblur operator=in /><feMerge><feMergeNode/><feMergeNode in=SourceGraphic /></feMerge></filter></defs><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"></polygon></svg> Powered by <a alt="upLoadly"href=https://github.com/JMcrafter26/upLoadly target=_blank>upLoadly</a></div>
	</body>
</html>